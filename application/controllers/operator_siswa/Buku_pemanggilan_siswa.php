...
class buku_pemanggilan_siswa extends MY_Controller {
	...
	public $arr = [
			...
			'table'				=>	'buku_pemanggilan_siswa',
			'column'			=>	[ 'idsiswa_fk', 'tanggal'],
			'column_order'		=>	[ 'id_buku_pemanggilan_siswa','idsiswa_fk', 'tanggal'],
			'column_search'		=>	[ 'id_buku_pemanggilan_siswa','idsiswa_fk', 'tanggal'],
	...
	*/
	public function get_data()
	{
		...
		if ($this->agent->is_mobile()) {
 			$this->my_view(['role/operator_siswa/page_mobile/buku_pemanggilan_siswa/index_page/index','role/operator_siswa/page_mobile/buku_pemanggilan_siswa/index_page/js'],$data);
 		}else{
 			$this->my_view(['role/operator_siswa/page/buku_pemanggilan_siswa/index_page/index','role/operator_siswa/page/buku_pemanggilan_siswa/index_page/js'],$data);
 		}
	...
	public function add_page()
	{
		...
		$this->my_view(['role/operator_siswa/page/buku_pemanggilan_siswa/add_page/index','role/operator_siswa/page/buku_pemanggilan_siswa/add_page/js'],$data);
	}
	...
	public function edit_page($id)
	{
		if (isset($id)) {
				...
				$data['jenis_ijin'] = 	$this->my_where('jenis_ijin', [])->result_array();
				$data['operator'] 	= 	$this->my_where('operator', ['id_operator'=>$this->get_user_account()['anggota_id']])->result_array();
				$data['siswa']		=	$this->my_where('siswa', ['id_siswa'=>$data['buku_pemanggilan_siswa']['idsiswa_fk']])->row_array();
				$this->my_view(['role/operator_siswa/page/buku_pemanggilan_siswa/edit_page/index','role/operator_siswa/page/buku_pemanggilan_siswa/edit_page/js'],$data);
		} else {
			$this->get_data();
		}
	}
	...
	/*
		ADD DATA
	*/
	function get_data_pemanggilan(){
		$data['param'] 		= 	$this->arr;
		$buku_pemanggilan_siswa			=	$this->db->query('SELECT * FROM `buku_pemanggilan_siswa` order by id_buku_pemanggilan_siswa desc limit 10')->result_array();
		foreach ($buku_pemanggilan_siswa as $key => $value) {
			...
			$data['buku_pemanggilan_siswa'][] = [
				'buku_pemanggilan_siswa'	=>	$value,
				'siswa' => $siswa,
				'kelas'	=>	$kelas,
			];
		}
		$this->my_view(['role/operator_siswa/page_mobile/buku_pemanggilan_siswa/index_page/get_data'],$data);
	}
	function get_siswa()
	{
	...
	*/
		...
		{
				$data['param'] 		= 	$this->arr;
				$data['buku_pemanggilan_siswa'] 	= $this->db->query('SELECT * FROM `buku_pemanggilan_siswa` where date(tanggal) = curdate()')->result_array();
				$data['template_buku_pemanggilan_siswa']	=
					[
						'header' => $this->generate_code($this->my_where('setting_table',['table'=>'buku_pemanggilan_siswa', 'name'=>'header_print_table'])->row_array()['value']),
						'footer' => $this->generate_code($this->my_where('setting_table',['table'=>'buku_pemanggilan_siswa', 'name'=>'footer_print_table'])->row_array()['value']),
						'title' => $this->generate_code($this->my_where('setting_table',['table'=>'buku_pemanggilan_siswa', 'name'=>'title_print_table'])->row_array()['value']),
					];
				$this->load->view('role/operator_siswa/page/buku_pemanggilan_siswa/print/index',$data);
		}
		function cetak_page()
		{
			$dt = $this->arr;
		...
		function cetak_data()
		{
...

		    {
				...
				$spreadsheet = $reader->load("include/template/excel/format_cetak_buku_pemanggilan_siswa.xlsx");
	...
	*/
	...
	public function datatable()
	{
       ...
       $this->db->where('idoperator_fk', $this->get_user_account()['anggota_id']);
        ...
        foreach ($list as $field) {
            ...
            // $row[]		=	'<a class="btn btn-xs btn-primary" href="'.base_url('operator_siswa/buku_pemanggilan_siswa/print_data/'.$field['id_buku_pemanggilan_siswa']).'" target="__blank"><i class=" icon-printer"></i></a>';
	...
	{
		$data['total']		=	$this->my_where('buku_pemanggilan_siswa', [])->num_rows();
		$data['hari_ini']	=	$this->my_where('buku_pemanggilan_siswa', ['tanggal'=>date('Y-m-d')])->num_rows();

		$this->my_view(['role/operator_siswa/page/buku_pemanggilan_siswa/index_page/stats'],$data);
	}
	function print_data($id)
	{
		...
		$data['template_buku_pemanggilan_siswa']	=
		[
			'header' => $this->generate_code($this->my_where('setting_table',['table'=>'buku_pemanggilan_siswa', 'name'=>'header_print_data'])->row_array()['value']),
			'footer' => $this->generate_code($this->my_where('setting_table',['table'=>'buku_pemanggilan_siswa', 'name'=>'footer_print_data'])->row_array()['value']),
			'title' => $this->generate_code($this->my_where('setting_table',['table'=>'buku_pemanggilan_siswa', 'name'=>'title_print_data'])->row_array()['value']),
		];
		
		$this->load->view('role/operator_siswa/page/buku_pemanggilan_siswa/print/print_data',$data);
	}
