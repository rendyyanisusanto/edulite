...
class kerjasama_dudi extends MY_Controller {
	

	public $arr = [
			'title'				=>	'Halaman kerjasama_dudi',
			'table'				=>	'kerjasama_dudi',
			'column'			=>	[ 'idjeniskerjasama_fk','iddudi_fk','keterangan', 'berkas'],
			'column_order'		=>	[ 'id_kerjasama_dudi','idjeniskerjasama_fk','iddudi_fk','keterangan', 'berkas'],
			'column_search'		=>	[ 'id_kerjasama_dudi','idjeniskerjasama_fk','iddudi_fk','keterangan', 'berkas'],
			'order'				=>	['id_kerjasama_dudi'	=>	'DESC'],
			'id'				=>	'id_kerjasama_dudi'
	];
	...
	*/
	public function get_data()
	{
		...
		$this->my_view(['role/admin/page/kerjasama_dudi/index_page/index','role/admin/page/kerjasama_dudi/index_page/js'],$data);
	}
	...
	public function add_page()
	{
		...
		$data['jenis_kerjasama']		=	$this->my_where('jenis_kerjasama',[])->result_array();

		$this->my_view(['role/admin/page/kerjasama_dudi/add_page/index','role/admin/page/kerjasama_dudi/add_page/js'],$data);
	}
	...
	public function edit_page($id)
	{
		if (isset($id)) {
				$data['param'] 		= 	$this->arr;
				$data['kerjasama_dudi'] = 	$this->my_where('kerjasama_dudi',['id_kerjasama_dudi'=>$id])->row_array();
				$data['dudi']		=	$this->my_where('dudi',[])->result_array();
				$data['jenis_kerjasama']		=	$this->my_where('jenis_kerjasama',[])->result_array();
				$this->my_view(['role/admin/page/kerjasama_dudi/edit_page/index','role/admin/page/kerjasama_dudi/edit_page/js'],$data);
		} else {
			// $this->get_data();
		}
	}
	...
	*/
	...
	{	
		$file_arsip = $this->save_media([
			'path'	=>	"./include/media/berkas_kerjasama_dudi/",
			'filename' => 'file_arsip',
		]);
		$data = [
			'idjeniskerjasama_fk' 		=> $_POST['idjeniskerjasama_fk'],
			'keterangan' 				=> $_POST['keterangan'],
			'iddudi_fk' 				=> $_POST['iddudi_fk'],
			'berkas'					=>	((isset($file_arsip)) ? $file_arsip['file_name'] : ''),
		];

		if ($this->save_data('kerjasama_dudi', $data)) {
			echo "Success";
		}
	}
	...
	*/
	...
	function update_data()
	{
		...
		if ($this->my_update('kerjasama_dudi',$data,['id_kerjasama_dudi'=>$_POST['id_kerjasama_dudi']])) {
	...
	*/
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            $no++;
			$jenis_kerjasama = $this->my_where('jenis_kerjasama',['id_jenis_kerjasama'=>$field['idjeniskerjasama_fk']])->row_array();
			$dudi = $this->my_where('dudi',['id_dudi'=>$field['iddudi_fk']])->row_array();
            $row        =   array();
            $row[]      =   '<input type="checkbox" name="get-check" value="'.$field['id_kerjasama_dudi'].'"></input>';
            $row[]		=	'<b>'.$dudi['nama'].'</b>';
            $row[]		=	$jenis_kerjasama['jenis_kerjasama'];
            $row[]		=	$field['keterangan'];
            $row[]      =   '<a target="__blank" href="'.base_url('include/media/berkas_kerjasama_dudi/'.$field['berkas']).'">Unduh Berkas</a>';
            $data[]     =   $row;
        }
        $output = array(
            "draw" => $_POST['draw'],
