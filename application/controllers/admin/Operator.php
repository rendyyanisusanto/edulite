...
class Operator extends MY_Controller {
	...
	public $arr = [
			...
			'table'				=>	'Operator',
			'column'			=>	[ 'kode', 'nama', 'keterangan', 'idlokasi_fk'],
			'column_order'		=>	[ 'id_Operator','kode', 'nama', 'keterangan', 'idlokasi_fk'],
			'column_search'		=>	[ 'id_Operator','kode', 'nama', 'keterangan', 'idlokasi_fk'],
			'order'				=>	['id_Operator'	=>	'DESC'],
			'id'				=>	'id_Operator'
	];
	...
	*/
	public function get_data()
	{
		...
		$data['param'] 		= 	$this->arr;
		$this->my_view(['role/admin/page/Operator/index_page/index','role/admin/page/Operator/index_page/js'],$data);

	}

	public function add_page()
	{
		$data['lokasi']	=	$this->my_where('lokasi', [])->result_array();
		$data['account']	=	$this->get_user_account();
		$data['param'] 		= 	$this->arr;
		$this->my_view(['role/admin/page/Operator/add_page/index','role/admin/page/Operator/add_page/js'],$data);
	}
	...
	*/
	...
	{	
		...
		if ($this->save_data('operator', $data)) {
			$this->get_data();
		}	else 	{
			echo "error";
		}
	...
	*/
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            $no++;
            $row        =   array();
            $lokasi 	=	$this->my_where('lokasi', ['id_lokasi'=>$field['idlokasi_fk']])->row_array();
           $row[]      =   '<input type="checkbox" name="get-check" value="'.$field['id_dudi'].'"></input>';
            $row[]		=	$field['kode'];
            $row[]		=	$field['nama'];
            $row[]		=	$field['no_hp'];
	...
	function get_jur(){
		...
		if ($id) {
			...
			$this->my_view(['role/admin/page/Operator/add_page/get_jur'], $data);
		}
	}
