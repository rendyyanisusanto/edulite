	...
	*/
	...
	function akun_get($data){
		...
		foreach ($akun as $key => $value) {
			...
			$cek = $this->db->select($select)->where('idakun_fk', $value['id_akun'])->where('DATE(create_at) >=', date_format(date_create($mulai),"Y/m/d"))->where('DATE(create_at) <=', date_format(date_create($sampai),"Y/m/d"))->get('jurnal_umum');
