...
class suplier extends MY_Controller {
	...
	public $arr = [
			...
			'table'				=>	'suplier',
			'column'			=>	['suplier','no_hp'],
			'column_order'		=>	[ 'id_suplier','suplier','no_hp'],
			'column_search'		=>	[ 'id_suplier','suplier','no_hp'],
			'order'				=>	['id_suplier'	=>	'DESC'],
			'id'				=>	'id_suplier'
	];
	...
	*/
	public function get_data()
	{
		...
		$data['param'] 		= 	$this->arr;
		$this->my_view(['role/koperasi/page/suplier/index_page/index', 'role/koperasi/page/suplier/index_page/js'],$data);
	}

	public function persebaran_suplier()
	{

		$data['account']	=	$this->get_user_account();
		$data['param'] 		= 	$this->arr;
		$data['suplier']		=	json_encode($this->my_where('suplier', [])->result_array());
		$this->my_view(['role/koperasi/page/suplier/persebaran_page/index','role/koperasi/page/suplier/persebaran_page/js'],$data);
	}
	public function add_page()
	{
		$data['account']	=	$this->get_user_account();
		$data['param'] 		= 	$this->arr;
		$this->my_view(['role/koperasi/page/suplier/add_page/index','role/koperasi/page/suplier/add_page/js'],$data);
	}
	...
	public function edit_page($id)
	{
		if (isset($id)) {
				$data['param'] 		= 	$this->arr;
				$data['suplier'] = 	$this->my_where('suplier',['id_suplier'=>$id])->row_array();
				$this->my_view(['role/koperasi/page/suplier/edit_page/index','role/koperasi/page/suplier/edit_page/js'],$data);
		} else {
			$this->get_data();
		}
	}
	...
	*/
	...
	{	
		...
		if ($this->save_data('suplier', $data)) {
			echo "Success";
		}
	}
	...
	*/
	...
	function update_data()
	{
		...
		if ($this->my_update('suplier',$data,['id_suplier'=>$_POST['id_suplier']])) {
	...
	*/
		...
		{
				$data['param'] 		= 	$this->arr;
				$data['suplier'] 	= $this->db->query('SELECT * FROM `suplier` where date(tanggal_surat) = DATE(NOW())')->result_array();
				$this->load->view('role/koperasi/page/suplier/print/print_surat',$data);
		}
		function cetak_page()
		{
			$dt = $this->arr;
			...
			$this->my_view(['role/koperasi/page/suplier/print_page/index','role/koperasi/page/suplier/print_page/js'],$data);
	...
	*/
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            $no++;
            $row        =   array();
            $row[]      =   '<input type="checkbox" name="get-check" value="'.$field['id_suplier'].'"></input>';
            $row[]		=	$field['suplier'];
            $row[]		=	$field['no_hp'];
            $data[]     =   $row;
        }
