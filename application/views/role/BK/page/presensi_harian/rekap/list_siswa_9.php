	...
	<table class="" style="overflow: scroll;width: 100%;">
		<thead>
			<tr>
				...
				<th class="text-table text-center" colspan="<?php echo iterator_count(($data_get['tg'])) * 2 ?>">Tanggal</th>

				<th class='text-table text-center' rowspan="3" style="width: 3% !important;">M</th>
				<th class='text-table text-center' rowspan="3" style="width: 3% !important;">TM</th>
			</tr>
			<tr>
				<?php foreach ($data_get['tg'] as $key => $value) { 
					echo "<th class='text-table text-center' colspan='2' width='25px'>".$value->format("d")."</th>";
				} ?>
			</tr>
			<tr>
				
				<?php foreach ($data_get['tg'] as $key => $value) { 
					echo "<th class='text-table text-center' width='25px'>M</th>";
					echo "<th class='text-table text-center' width='25px'>P</th>";
				} ?>
			</tr>
		</thead>
		<tbody>
			...
			<tr>
				...
				// print_r($value2['presensi']);  
					...
					if (isset($value2['presensi_pulang'])&&(!empty($value2['presensi_pulang']))) {
						if ($value2['presensi_pulang'] == "M") {
								echo "<td width='25px' class='text-table text-center'><b class='text-success'>√</b></td>";
						}elseif ($value2['presensi'] == "A") {
								echo "<td width='25px' class='text-table text-center'><b class='text-danger'>X</b></td>";
						}else{
								echo "<td width='25px' class='text-table text-center'><b class='text-info'>".$value2['presensi_pulang']."</b></td>";
						}	
					}else{
						echo "<td width='25px' class='text-table'></td>";
					}
