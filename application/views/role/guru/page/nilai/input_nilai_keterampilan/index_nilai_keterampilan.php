<div class="panel panel-body">
	<center><h3>Nilai keterampilan </h3></center>
			<div class="alert alert-warning">
				Jika anda lebih familiar menggunakan ms excel silahkan <a href="<?php echo base_url('guru/nilai/download_file_keterampilan/'.$data_get['kelas']['id_kelas'].'/'.$data_get['mata_pelajaran']['id_mata_pelajaran'].'/'.$data_get['tahun_ajaran']['id_tahun_ajaran'].'/'.$data_get['kd']['id_kd'].'/'.$data_get['jenis_keterampilan']['id_jenis_keterampilan']) ?>" target="__blank">download format ini</a>, kemudian lakukan import data pada form dibawah<br>
				<form action="<?php echo $data_get['param']['table'] ?>/import_keterampilan/<?php echo $data_get['kd']['id_kd'] ?>" id="app-import" method="POST">
					<input type="hidden" name="idmatapelajaran_fk" value="<?php echo $data_get['mata_pelajaran']['id_mata_pelajaran']; ?>" class="form-control input-xs">
							...
							<input type="hidden" name="idjenisketerampilan_fk" value="<?php echo $data_get['jenis_keterampilan']['id_jenis_keterampilan']; ?>" class="form-control input-xs">
			...
			<table class="table table-bordered table-framed" style="width: 100%;" >
		        ...
		        <tr>
		          <td class="bg-slate">KD 4</td>
		          <td class="bg-slate-300"><?php echo $data_get['kd']['ringkasan'] ?></td>
		        </tr>
		        <tr>
		          <td class="bg-slate">Jenis Penilaian</td>
		          <td class="bg-slate-300"><?php echo $data_get['jenis_keterampilan']['jenis_keterampilan'] ?></td>
		        </tr>
		      </table>
		      <hr>
	<table style ="border: 1px solid black;" class="table table-bordered">
				<thead>
					<tr>
						...
						<th style ="border: 1px solid black;" class="bg-blue" width="20%" rowspan="2">NK</th>
					</tr>
				</thead>
				<tbody>
					<?php 
