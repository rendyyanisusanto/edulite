...
<div class="panel panel-body">
	<center><h3>Presensi Siswa Mapel <?php echo $data_get['mata_pelajaran']['mata_pelajaran'].' Kelas '.$data_get['kelas']['kelas'] ?> Periode <?= $data_get['bulan'] ?></h3></center>
				...
				<?php for ($i=1; $i <= $data_get['all_day'] ; $i++) { 
					...
					if ((isset($value['presensi'][$i]['presensi']))) {
						if ($value['presensi'][$i]['presensi'] == "M") {
								...
								echo "<td width='25px' class='text-table text-center bg-danger'><b class=''>X</b></td>";
						}else{
								echo "<td width='25px' class='text-table text-center bg-info'><b>".$value['presensi'][$i]['presensi']."</b></td>";
						}	
					}else{
