<script type="text/javascript">
	$(".alumni").select2({
	         ajax: { 
	           url: '<?php echo base_url('admin/Transaksi_tanggungan_alumni/get_alumni') ?>',
	           type: "post",
	           dataType: 'json',
	           delay: 250,
	...
	$('.alumni').on('change', function(e){
		e.stopImmediatePropagation()
		clear_table();
	})
	function add_cart(kode) {
		// 
		send_ajax('Transaksi_tanggungan_alumni/add_row', {id:$('.alumni').val()}).then(function(data){
	...
	$('.btn-add').on('click', function(e){
		...
		if($('.alumni').val() !== null)
		{
			...
			toastr.error('Pilih alumni terlebih dahulu');
		}
	})
