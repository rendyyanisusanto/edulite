<tr id="tr<?php echo $data_get['rand']; ?>">
	...
	<td>
		<!--  -->
		<select name="detail[<?php echo $data_get['rand']; ?>][idtanggunganalumni_fk]" data-row="<?php echo $data_get['rand']; ?>" class="form-control select-tanggungan">
			<?php foreach ($data_get['tanggungan'] as $value): ?>
				<option data-jumlah="<?= number_format($value['jumlah'],0,'.','.'); ?>" data-sisa="<?= number_format(($value['jumlah'] - $value['pembayaran']), 0, '.', '.'); ?>" value="<?= $value['id_tanggungan_alumni'] ?>"><?= $value['keterangan'] ?></option>
			<?php endforeach ?>
		</select>
	</td>
