<div class="panel panel-body panel-indikator">
	...
	<form class="form-horizontal" action="<?php echo $data_get['param']['table'] ?>/save_indikator" id="app-indikator" method="POST">
		<div class="form-group">
			...
			<div class="col-lg-8">
				<input type="text" required="" class="form-control" name="kompetensi">
	...
	<table class="table table-framed table-xxs table-indikator">
		...
		<tbody>
			...
			<?php foreach ($data_get['indikator_pkg'] as $key => $value): ?>
				<tr>
					<td><button class="btn btn-danger btn-del btn-xs" data-id="<?php echo $value['id_kompetensi_pkg'] ?>"><i class="icon-trash"></i></button></td>
					<td><?php echo $value['kompetensi'] ?></td>
					<td><button class="btn btn-success btn-sub btn-xs" data-id="<?php echo $value['id_kompetensi_pkg'] ?>"><i class="icon-circle-right2"></i></button></td>
				</tr>
			<?php endforeach ?>
		</tbody>
	</table>
